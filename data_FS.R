...
data_FS <- function(alpha, power, diff=0.2, p0.min=0.05, p0.max=0.7, p0.incre=0.005) {
  ...
  for(p0 in P0) {
    ...
    op.design <- rbind(op.design, design_FS(alpha, power, p0 = p0, p1 = p1, type = "optimal", max.N = 70))
    mm.design <- rbind(mm.design, design_FS(alpha, power, p0 = p0, p1 = p1, type = "minimax", max.N = 70))
